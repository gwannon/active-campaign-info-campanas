...
function generateTable(json) {
  json.forEach(function(data, index) {
    jQuery("table tbody").append("<tr>"+
      "<td><a href='./view.php?id="+data.id+"' target='_blank'>"+data.name+"</a>"+(data.segment_name != '' ? "<br/><span style='font-size: 10px;'>"+data.segment_name+"</span>" : "")+"<div class='image'><img src='"+data.image+"' /></div></td>"+
      "<td>"+data.date+"</td>"+
      "<td>"+data.subject+"</td>"+
      "<td>"+data.send_amt+"</td>"+
